...
namespace pacs {
    ...
    /**
     * @brief Returns the modal coefficients of a function evaluated over the boundary.
     * 
     * @param mesh 
     * @param function 
     * @return Vector<Real> 
     */
    Vector<Real> modal_boundary(const Mesh &mesh, const Functor &function) {
        ...
        for(std::size_t j = 0; j < mesh.elements.size(); ++j) {
            ...
            for(std::size_t k = 0; k < element_neighbours.size(); ++k) {
                ...
                // Boundary conditions.
                Vector<Real> boundary = function(physical_x, physical_y);

                // Local forcing term.
                local_coefficients += scaled_phi.transpose() * boundary;
            }
