 ...
 * @brief Poisson on a square domain. Element size adaptive refinement. H1 error.
 * @date 2024-07-02
 * 
...
int main(int argc, char **argv) {
    ...
    // "Splash".
    std::ofstream output{"output/square_gh_" + std::to_string(elements) + "@" + std::to_string(degree) + ".error"};
    ...
    for(std::size_t index = 0; index < TESTS_MAX; ++index) {
        ...
        // Mesh output.
        std::string polyfile = "output/square_gh_" + std::to_string(elements) + "@" + std::to_string(degree) + "_" + std::to_string(index) + ".poly";
        mesh.write(polyfile);
